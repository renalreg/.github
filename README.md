...
- <a href="#docker_publish">**UKKA Docker Publish**</a>
...
<a id="docker_publish"></a>

## Docker Publish

Builds a Docker image and pushes it to the GitHub Container Registry so it can be pulled onto servers. Runs on published releases, pushes to the default branch, PRs against the default branch, and can be manually triggered. Release Please branches are skipped.

The image is always named after the repository, at `ghcr.io/renalreg/<repo-name>`. Tags are generated by the metadata action:

- `1.2.3` and `1.2` on a semver release tag
- `latest` on the default branch
- the branch name or PR number otherwise

The Dockerfile is expected at the repository root. There is no checkout step: the build runs against the Git context, so Buildx fetches the repo itself.

Pushing uses the built in `GITHUB_TOKEN` with `packages: write`, so no extra secret is needed for the registry itself.

### Private dependencies

Set `org_deps: true` when the image installs packages from private UKKA repos. A GitHub App token is generated and passed to the build as a **BuildKit secret** named `github_token` rather than a build arg, so it is never baked into the image layers or history. Consume it in the Dockerfile like so:
...
with:
      ...
      # Default: ""
      org_deps: true
        # Description: Boolean, is private repo access required by dependencies
        # Default: false
```
...
docker pull ghcr.io/renalreg/my-repo:latest
```
